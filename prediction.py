...
class PredictionLayer(nn.Module):
    '''
        ...
        - SUM
    '''
    def __init__(self, user_num, item_num, feature_dim, output):
        super(PredictionLayer, self).__init__()
        self.output = output
        
        if output == "fm":
            self.model = FM(feature_dim, user_num, item_num)
        elif output == "lfm":
            self.model = LFM(feature_dim, user_num, item_num)
        elif output == 'mlp':
            self.model = MLP(feature_dim)
        elif output == 'nfm':
            self.model = NFM(feature_dim)
        else:
            self.model = torch.sum

    def forward(self, feature, uid, iid):
        if self.output == "lfm" or "fm" or "nfm":
            # uid = uid.type(torch.long)
            # iid = iid.type(torch.long)
